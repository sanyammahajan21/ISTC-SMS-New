...
const GeneratediplomaCertificatesPage = () => {
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0);

  const downloadAlldiplomaCertificates = async () => {
    try {
      setLoading(true);
      setProgress(0);

      // Fetch all 8th semester students
      const response = await fetch("/api/studentsfordiploma");
      ...
      for (let i = 0; i < totalStudents; i++) {
        ...
        const pdfResponse = await fetch(`/api/certficates/diploma?studentId=${student.id}`);
        ...
        zip.file(`${student.name}_diploma_certificate.pdf`, pdfBlob);
      ...
      saveAs(zipContent, "diploma_certificates.zip");
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Generate diploma Certificates</h2>
      <p className="mb-6 text-gray-600">
        Download diploma certificates for all eligible students.
      </p>
      
      <button
        onClick={downloadAlldiplomaCertificates}
        disabled={loading}
...
export default GeneratediplomaCertificatesPage;