...
const GenerateMigrationCertificatesPage = () => {
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0);

  const downloadAllMigrationCertificates = async () => {
    try {
      setLoading(true);
      setProgress(0);
      ...
      for (let i = 0; i < totalStudents; i++) {
        ...
        const pdfResponse = await fetch(`/api/certficates/migration?studentId=${student.id}`);
        ...
        zip.file(`${student.name}_migration_certificate.pdf`, pdfBlob);
      ...
      saveAs(zipContent, "migration_certificates.zip");
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Generate Migration Certificates</h2>
      <p className="mb-6 text-gray-600">
        Download migration certificates for all students in the 8th semester.
      </p>
      
      <button
        onClick={downloadAllMigrationCertificates}
        disabled={loading}
...
export default GenerateMigrationCertificatesPage;