...
const GenerateCharacterCertificatesPage = () => {
  const [loading, setLoading] = useState(false);
  const [progress, setProgress] = useState(0);

  const downloadAllCharacterCertificates = async () => {
    try {
      setLoading(true);
      setProgress(0);
      ...
      for (let i = 0; i < totalStudents; i++) {
        ...
        const pdfResponse = await fetch(`/api/certficates/character?studentId=${student.id}`);
        ...
        zip.file(`${student.name}_character_certificate.pdf`, pdfBlob);
      ...
      saveAs(zipContent, "character_certificates.zip");
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Generate character Certificates</h2>
      <p className="mb-6 text-gray-600">
        Download character certificates for all students in the 8th semester.
      </p>
      
      <button
        onClick={downloadAllCharacterCertificates}
        disabled={loading}
...
export default GenerateCharacterCertificatesPage;